...
def generate_launch_description():
    ...
    # Foxy 的 ComposableNode 不会自己读 YAML 文件（launch_ros #156），
    # 这里在 launch 里解析好，以 dict 形式传进去
    with open(node_params, 'r') as f:
        all_params = yaml.safe_load(f)
    camera_params = all_params['/camera_node']['ros__parameters']
    detector_params = all_params['/armor_detector']['ros__parameters']
